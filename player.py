...
# Create a lock to synchronize access to the input prompt
lock = threading.Lock()

def receive_messages():
    global players_info, cards, cards_facing_up, discard_pile
    while True:
        message, addr = sock_player.recvfrom(1024)
        message = message.decode('utf-8')

        # Acquire lock before processing the message
        with lock:
            ...
            if message.startswith("PLAYER_INFO"):
                ...
                sock_player.sendto(discard_pile_top.encode('utf-8'), addr)
